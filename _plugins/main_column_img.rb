  ...
  class RenderMainColumnTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @text[0].start_with?('http://', 'https://','//')
        "<figure><figcaption><span class='marginnote'>#{@text[1]}</span></figcaption><img src='#{@text[0]}'/></figure>"
      else
        "<figure><figcaption><span class='marginnote'>#{@text[1]}</span></figcaption><img src='#{baseurl}/#{@text[0]}'/></figure>"
      end
    end
  end
end
